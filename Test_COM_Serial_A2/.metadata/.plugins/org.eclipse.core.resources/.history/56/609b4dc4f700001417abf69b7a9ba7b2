/*===========================================================================
 ...
 1. Define the port of the Master as 4A and 4B and burn it on the Node. Define en_master as "1"
 2. Define en_master as "0". Change the definition of the ports for the Slaves to 4A and 4B and burn it on the Node.

 ! Don't foget to supply it with 12V !
...
on tile[0]: out port p_master_txd = XS1_PORT_4A;
on tile[0]: in port p_master_rxd = XS1_PORT_4B;

on tile[0]: out  port en_master = XS1_PORT_1F;

#if ISBUS_NODE_COUNT == 2
on tile[0]: out port p_slave_txd = XS1_PORT_4E;
on tile[0]: in port p_slave_rxd = XS1_PORT_4F;
#endif
...
on tile[0]: in port p_master_shadow = XS1_PORT_8A;
on tile[0]: in port p_slave_shadow = XS1_PORT_8C;
on tile[0]: out port p_led0_slave = XS1_PORT_1K;
on tile[0]: out port p_led0_master = XS1_PORT_1C;
on tile[0]: out port p_led1_slave = XS1_PORT_1M;
on tile[0]: out port p_led1_master = XS1_PORT_4C;
...
int main() {
  ...
  par {
    on tile[0]: master_application(p_master_txd, c_ma2s);
    on tile[0]: lin_rx_server(p_master_rxd, c_ma2s);
#if ISBUS_NODE_COUNT == 2
    on tile[0]: slave_application(p_slave_txd, c_sa2s);
    on tile[0]: lin_rx_server(p_slave_rxd, c_sa2s);
#endif
    on tile[0]: dso_led_app();              //Optional monitor task for debug purposes
  }
  return 0;
}
